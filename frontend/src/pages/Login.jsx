        ...
        <div className="relative z-10 flex h-full flex-col justify-between p-10 xl:p-14">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] font-medium !text-slate-300">Employee Document Management</p>
            </div>
          </div>
          ...
          <div className="max-w-xl">
            ...
            <h1 className="text-5xl font-extrabold leading-[1.08] tracking-[-0.045em] !text-white xl:text-6xl">
              Your documents.
              <br />
              <span className="!text-indigo-400">Securely managed.</span>
            </h1>

            <p className="mt-6 max-w-lg text-base leading-7 !text-slate-300">
              Store, access and manage official employee documents through one centralized workspace
              built for secure organizational document management.
            </p>

            <div className="mt-9 grid gap-3 sm:grid-cols-2">
              <Feature title="Centralized" description="One organized document workspace" />
              <Feature title="Role-based" description="Access based on your account" />
              <Feature title="Secure access" description="Protected employee documents" />
              <Feature title="Easy management" description="View, upload and download" />
            </div>
          </div>

          <div className="flex items-center justify-between border-t border-white/[0.08] pt-6">
            <p className="text-[10px] font-medium !text-slate-400">Enterprise Document Management System</p>
            <p className="text-[10px] font-medium !text-slate-400">© 2026 EDMS</p>
          </div>
        </div>
      </section>
        ...
        <div className="w-full max-w-[430px]">
          ...
          <div className="mb-8 flex items-center justify-center lg:hidden">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-bold tracking-tight text-slate-900">EDMS</p>
                <p className="text-[10px] font-medium text-slate-400">Employee Documents</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-[24px] border border-slate-200 bg-white p-6 shadow-[0_20px_60px_-30px_rgba(15,23,42,0.25)] sm:p-8">
            <div>
              ...
              <h2 className="text-[27px] font-bold tracking-[-0.04em] text-slate-950">Welcome back</h2>
              <p className="mt-2 text-sm leading-6 text-slate-500">
                Sign in to access your employee document workspace.
              </p>
            </div>
            ...
            {error && (
              <div role="alert" className="mt-5 flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3.5">
                ...
                <p className="text-xs font-semibold leading-5 text-red-700">{error}</p>
              </div>
            )}

            <form onSubmit={handleSubmit} className="mt-7 space-y-5">
              <div>
                <label htmlFor="email" className="mb-2 block text-[11px] font-bold uppercase tracking-[0.07em] text-slate-600">
                  Email address
                </label>
                <div className="group relative">
                  <Mail size={17} className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 transition group-focus-within:text-indigo-500" />
                  <input
                    id="email"
                    name="email"
                    type="email"
              ...
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <label htmlFor="password" className="block text-[11px] font-bold uppercase tracking-[0.07em] text-slate-600">
                    Password
                  </label>
                  <Link to="/forgot-password" className="text-[11px] font-semibold text-indigo-600 transition hover:text-indigo-700 hover:underline">
                    Forgot password?
                  </Link>
                </div>
                <div className="group relative">
                  <Lock size={17} className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 transition group-focus-within:text-indigo-500" />
                  <input
                    id="password"
                    name="password"
            ...
            <div className="mt-6 flex items-start gap-3 rounded-xl border border-slate-100 bg-slate-50 p-3.5">
              ...
              <div>
                <p className="text-[11px] font-bold text-slate-700">Secure workspace access</p>
                <p className="mt-0.5 text-[10px] leading-4 text-slate-400">
                  Your account permissions determine which employee workspace you can access.
                </p>
              </div>
            </div>
            ...
            <div className="mt-7 border-t border-slate-100 pt-6 text-center">
              <p className="text-xs text-slate-500">
                ...
                <Link to="/register" className="font-bold text-indigo-600 transition hover:text-indigo-700">
                  Create an account
                </Link>
              </p>
            </div>
...
function Feature({ title, description }) {
  return (
    <div className="rounded-xl border border-white/[0.10] bg-white/[0.045] p-3.5">
      ...
      <p className="mt-1 pl-[22px] text-[10px] leading-4 !text-slate-300">{description}</p>
    </div>
  );
}