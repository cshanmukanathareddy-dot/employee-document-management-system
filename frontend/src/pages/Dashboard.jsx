...
export default function Dashboard() {
  ...
  const accountStatus = user?.status
    ...
    : "Active";
  const isReadOnlyEmployee = user?.role === "readonlyemployee";
  ...
  const loadStorage = async () => {
    ...
    try {
      ...
      setStorageError(err.response?.data?.detail || "Unable to load storage information.");
    } finally {
  ...
  const submitStorageRequest = async () => {
    ...
    try {
      ...
      setStorageMessage("");
      setStorageError("");
      const response = await api.post("/storage/request", { additional_gb: amount });
      setStorageMessage(response.data.message);
      setStorageRequest("");
      await loadStorage();
    } catch (err) {
      setStorageError(err.response?.data?.detail || "Unable to submit storage request.");
    } finally {
          ...
          <div className="mx-auto max-w-[1500px]">
              ...
              <div className="relative z-10 flex flex-col gap-8 px-7 py-8 sm:px-8 lg:flex-row lg:items-center lg:justify-between lg:px-9 lg:py-9">
                <div className="max-w-2xl">
                  <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/[0.08] px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.1em] !text-indigo-100">
                    <Sparkles size={12} />
                    {isReadOnlyEmployee ? "Read-only workspace" : "Employee workspace"}
                  </div>
                ...
                <div className="relative shrink-0">
                  ...
                  <div className="absolute -bottom-3 -left-5 flex items-center gap-2 rounded-xl border border-slate-200/80 bg-white px-3 py-2 shadow-xl">
                    ...
                    <div>
                      <p className="text-[10px] font-bold text-slate-800">Account secure</p>
                      <p className="text-[9px] text-slate-400">Workspace protected</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <section className="mt-6">
              <div className="mb-3 flex items-center justify-between">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400">Overview</p>
                  <h2 className="mt-1 text-base font-bold tracking-[-0.02em] text-slate-900">Your workspace</h2>
                </div>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                ...
                <OverviewCard
                  ...
                  title="Directory"
                  value={user?.directory_name || "Not assigned"}
            ...
            {/* STORAGE SECTION (IF NOT READ-ONLY) */}
            {!isReadOnlyEmployee && (
              <section className="mt-6 overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-sm">
                <div className="flex flex-col gap-5 px-6 py-5 lg:flex-row lg:items-center lg:justify-between">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h2 className="text-base font-bold tracking-[-0.015em] text-slate-950">Document storage</h2>
                      <p className="mt-1 text-sm text-slate-500">Your employee document storage allocation and usage.</p>
                    </div>
                  </div>

                  {storageLoading ? (
                    <div className="text-sm text-slate-400">Loading storage...</div>
                  ) : storage ? (
                    <div className="w-full max-w-xl">
                      <div className="mb-2 flex items-center justify-between text-xs">
                        <span className="font-semibold text-slate-600">
                          {formatStorageBytes(storage.storage_used_bytes)} used
                        </span>
                        <span className="font-bold text-slate-800">
                          {formatStorageBytes(storage.storage_limit_bytes)} allocated
                        </span>
                      </div>
                          ...
                          style={{
                            width: `${Math.min(
                              storage.storage_limit_bytes > 0
                                ? (storage.storage_used_bytes / storage.storage_limit_bytes) * 100
                                : 0,
                              100
                            )}%`,
                          }}
                      ...
                      <p className="mt-2 text-xs text-slate-400">
                        {formatStorageBytes(storage.storage_remaining_bytes)} remaining
                      </p>
                    </div>
                  ) : null}
                </div>
                  ...
                  {storage?.pending_request ? (
                    <div className="flex flex-col gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 sm:flex-row sm:items-center sm:justify-between">
                      <span className="font-semibold">
                        Pending request: +{formatStorageGb(storage.pending_request.additional_gb)} additional storage
                      </span>
                      <span className="text-xs text-amber-600">Waiting for administrator approval</span>
                    </div>
                  ) : (
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-2 mb-3">
                        {REQUEST_STORAGE_PRESETS.map((p) => (
                          <button
                            key={p.label}
                            type="button"
                            onClick={() => setStorageRequest(String(p.gb))}
                            className={`rounded-lg px-3 py-1.5 text-xs font-bold transition ${
                              storageRequest === String(p.gb)
                      ...
                      <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
                        <div className="flex-1">
                          <input
                            ...
                            onChange={(e) => setStorageRequest(e.target.value)}
                            placeholder="e.g. 0.5 (for 500 MB) or 1 (for 1 GB)"
                        ...
                        >
                          <HardDrive size={16} />
                          {storageSubmitting ? "Submitting..." : "Request Storage"}
                        </button>
                      </div>

                      <p className="mt-2 text-xs text-slate-400">
                        Enter amount in GB (e.g., 0.5 = 500 MB, 1 = 1 GB). Requests are reviewed and approved by the administrator.
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div className="mt-6 grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
              <section className="overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-sm">
                <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
                  <div>
                    <h2 className="text-base font-bold tracking-[-0.015em] text-slate-950">Quick actions</h2>
                    <p className="mt-1 text-xs text-slate-500">Frequently used workspace actions</p>
                  </div>
              ...
              <section className="overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-sm">
                <div className="border-b border-slate-100 px-6 py-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-base font-bold tracking-[-0.015em] text-slate-950">Account information</h2>
                      <p className="mt-1 text-xs text-slate-500">Your current employee details</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="divide-y divide-slate-100 px-6">
                  ...
                  <Info label="Directory" value={user?.directory_name} />
                  <Info label="Account status" value={accountStatus} status={user?.status} />
                </div>
            ...
            <section className="mt-5 flex flex-col gap-4 rounded-[20px] border border-slate-200 bg-white px-5 py-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-bold text-slate-700">Secure employee workspace</p>
                  <p className="mt-0.5 text-[11px] text-slate-400">
                    Keep your information accurate and your documents up to date.
                  </p>
                </div>
              </div>
...
function OverviewCard({ icon, iconStyle, title, value, description, href, status = false }) {
  return (
    <Link
      to={href}
    ...
    >
      ...
      <div className="mt-5">
        ...
        <div className="mt-1.5 flex items-center gap-2">
          {status && <span className="h-2 w-2 rounded-full bg-emerald-500" />}
...
function ActionCard({ href, icon, iconStyle, title, description }) {
  return (
    <Link
      to={href}
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-bold text-slate-800 transition group-hover:text-indigo-700">{title}</p>
        <p className="mt-0.5 truncate text-[11px] text-slate-400">{description}</p>
      </div>
      <ArrowRight size={15} className="shrink-0 text-slate-300 transition group-hover:translate-x-0.5 group-hover:text-indigo-500" />
    </Link>
  );
}
  ...
  return (
    <div className="flex min-h-[58px] items-center justify-between gap-5">
      ...
      <div className="flex min-w-0 items-center justify-end gap-2">
        ...
        <span title={displayValue} className="max-w-[220px] truncate text-right text-xs font-bold text-slate-700">
          {displayValue}
        </span>
      </div>
    </div>
