...
export default function AdminDocuments() {
  ...
  const [isDeleting, setIsDeleting] =
    useState(false);
  const [copiedDocId, setCopiedDocId] =
    useState(null);
  ...
  function handleAdminDelete(doc) {
    if (!doc) {
      return;
    }
  ...
  function handleView(document) {
    ...
    if (document.is_archive && !document.extracted) {
      setError("ZIP files cannot be viewed directly. Click 'Extract' first to view its contents.");
      return;
    }

    const directory = document.directory_name || document.storage_directory || "admin";
    const url = buildPublicDocumentUrl(directory, document.document_name);
    if (!url) {
  ...
  async function handleCopyUrl(document) {
    if (!document) return;
    const directory = document.directory_name || document.storage_directory || "admin";
    const url = buildPublicDocumentUrl(directory, document.document_name);
    if (!url) {
      setError("Could not build the document URL.");
      return;
    }
  ...
  async function handleDownload(document) {
    ...
    try {
      setError("");
      const response = await api.get(`/admin-documents/${document.id}/download`, {
        responseType: "blob",
      });
      triggerBlobDownload(
        response,
      ...
      <main className="ml-0 lg:ml-64">
        ...
        <div className="p-4 sm:p-6 lg:p-8">
          ...
          <div className="mb-8 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="inline-flex w-fit items-center gap-2 rounded-xl border border-emerald-200 dark:border-emerald-800/60 bg-emerald-50 dark:bg-emerald-950/40 px-3.5 py-2.5 text-xs font-bold text-emerald-700 dark:text-emerald-300">
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_330px]">
            ...
            >
              ...
              <div className="border-b border-slate-100 dark:border-slate-800 bg-gradient-to-r from-white to-indigo-50/40 dark:from-slate-900 dark:to-indigo-950/40 px-7 py-6">
                ...
                <div className="mb-7">
                  ...
                  {!file ? (
                    <button
                      ...
                      className="group flex w-full flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-900/60 px-6 py-9 text-center transition hover:border-indigo-300 dark:hover:border-indigo-600 hover:bg-indigo-50/30 dark:hover:bg-indigo-950/30"
                    >

                      <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-sm ring-1 ring-slate-100 dark:ring-slate-700 transition group-hover:scale-105">
                        <Upload
                          size={21}
                        />
                      </div>
                      ...
                      <span className="mt-4 rounded-lg bg-white dark:bg-slate-800 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.08em] text-slate-500 dark:text-slate-400 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700">
                        Select file
                      </span>

                    </button>
                  ) : (
                    <div className="rounded-2xl border border-indigo-100 dark:border-indigo-900/40 bg-indigo-50/40 dark:bg-indigo-950/40 p-4">
            ...
            <aside className="space-y-5">
              ...
              <div className="rounded-2xl border border-indigo-100 dark:border-indigo-900/40 bg-indigo-50/50 dark:bg-indigo-950/40 p-5">

                <div className="flex items-center gap-3">

                  <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-white dark:bg-indigo-900/60 text-indigo-600 dark:text-indigo-300 shadow-sm">
                    <Users
                      size={17}
                    />
                  </div>
                  ...
                  .map((document) => (
                    ...
                    >
                      ...
                      <div className="flex shrink-0 items-center gap-2">
                        ...
                        {(!document.is_archive || document.extracted) && (
                          <>
                            <button
                              ...
                              title={copiedDocId === document.id ? "URL copied!" : "Copy clean document URL"}
                              className="flex h-10 w-10 items-center justify-center rounded-xl border border-indigo-100 bg-white text-indigo-600 transition hover:border-indigo-200 hover:bg-indigo-50"
                            >
                              {copiedDocId === document.id ? (
                            ...
                            <button
                              ...
                              title="View document"
                              className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-700 transition hover:border-indigo-200 hover:bg-slate-50"
                            >
                              <Eye size={15} />
                            </button>
                        ...
                        <button
                          type="button"
                          onClick={() => handleAdminDelete(document)}
                          title={
                            document.is_archive && document.extracted
                              ? "Delete folder"
                              : "Delete document"
                          }
                          className="flex h-10 w-10 items-center justify-center rounded-xl border border-red-100 bg-white text-red-500 transition hover:border-red-200 hover:bg-red-50"
                        >
                          <Trash2 size={15} />
                        </button>
                      </div>
...
function AdminDeleteConfirmModal({
  ...
  const displayName = document.document_name.replace(
    /\.zip$/i,
    ""
  );
        ...
        <div className="mt-4 rounded-xl bg-slate-50 border border-slate-200/80 p-3.5">
          ...
          <p className="mt-1 text-[11px] text-slate-500">
            {isFolder
              ...
              : "This document will be permanently removed from administrator storage."}
          </p>
        </div>
...
function PermissionCard({
  ...
  return (
    <label
      className={`group flex cursor-pointer items-start gap-3 rounded-xl border p-4 transition ${checked
    ...
    >
      ...
      <span
        className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg transition ${checked
...
function SummaryRow({
  ...
  return (
    <div className="flex items-start justify-between gap-4 border-b border-slate-100 pb-3 last:border-0 last:pb-0">
      ...
      <span
        className={`max-w-[60%] break-words text-right text-xs font-bold ${positive
            ? "text-emerald-600"
            : "text-slate-700"
          }`}
      >
