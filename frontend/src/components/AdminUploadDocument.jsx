...
export default function AdminUploadDocument({ onClose, onUploaded }) {
  ...
  const [permissions, setPermissions] = useState({ view: true, download: true });
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      formData.append("can_view", permissions.view);
      formData.append("can_download", permissions.download);
      ...
      setDepartment("");
      setEmployeeId("");
      setPermissions({ view: true, download: true });
      ...
      <div className="w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-5">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Upload and control document access.</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        {/* FORM */}
        <form onSubmit={handleUpload} className="max-h-[75vh] overflow-y-auto p-6">
          {error && (
          ...
          <div>
            ...
            <label className="flex cursor-pointer items-center gap-4 rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 p-5 transition hover:border-indigo-400 hover:bg-indigo-50">
              ...
              <div className="min-w-0">
                <p className="text-sm font-semibold text-slate-800">{file ? file.name : "Choose a document"}</p>
                <p className="mt-1 text-xs text-slate-500">Click to browse files</p>
              </div>
          ...
          {/* ACCESS TYPE */}
          <div className="mt-6">
            <label className="mb-3 block text-sm font-semibold text-slate-700">Access</label>
            <div className="space-y-3">
              <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 p-4 transition hover:bg-slate-50">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Make this document available to all employees.</p>
                </div>
              </label>
              ...
              <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 p-4 transition hover:bg-slate-50">
                <input
                  ...
                  name="access"
                  value="department"
                  checked={access === "department"}
                  onChange={() => setAccess("department")}
                  className="h-4 w-4 accent-indigo-600"
                />
                <div>
                  <p className="text-sm font-semibold text-slate-800">Department</p>
                  <p className="text-xs text-slate-500">Make this document available to one department.</p>
                </div>
              </label>
              ...
              <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 p-4 transition hover:bg-slate-50">
                <input
                  ...
                  onChange={() => setAccess("employee")}
                  className="h-4 w-4 accent-indigo-600"
                />
                <div>
                  <p className="text-sm font-semibold text-slate-800">Specific Employee</p>
                  <p className="text-xs text-slate-500">Make this document available to one employee.</p>
                </div>
              </label>
            </div>
          </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 p-4 hover:bg-slate-50">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Users can open the document.</p>
                </div>
              </label>
              ...
              <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 p-4 hover:bg-slate-50">
                ...
                />
                <div>
                  <p className="text-sm font-semibold text-slate-800">Download</p>
                  <p className="text-xs text-slate-500">Users can download the document.</p>
                </div>
              </label>
            </div>
          </div>
