      ...
      <div className="flex h-full items-center justify-between gap-2 pl-16 pr-3 sm:gap-4 sm:pr-6 lg:gap-5 lg:px-8">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-0.5 truncate text-xs text-slate-400">
            Welcome back, <span className="font-semibold text-slate-500">{firstName}</span>
          </p>
        </div>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          {showSearch ? (
            <form onSubmit={handleSearch} className="flex items-center">
              <div className="flex h-10 w-[min(280px,calc(100vw-88px))] items-center rounded-xl border border-indigo-200 bg-white px-3 shadow-[0_4px_16px_rgba(79,70,229,0.08)] ring-4 ring-indigo-50">
                <Search size={17} className="shrink-0 text-indigo-500" />
                <input
                  ref={searchInputRef}
                  autoFocus
                  type="text"
            ...
            >
              <Search size={17} className="transition-colors group-hover:text-indigo-500" />
          ...
          <div ref={notificationRef} className="relative">
            ...
            >
              <Bell size={19} strokeWidth={showNotifications ? 2.3 : 2} />
              {notificationCount > 0 && (
                <>
              ...
              <div className="absolute right-0 top-[52px] w-[min(390px,calc(100vw-24px))] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_20px_60px_rgba(15,23,42,0.15)]">
                <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
                  <div>
                    ...
                    <p className="mt-1 text-[11px] text-slate-400">Updates about your documents</p>
                  </div>
                  {notificationCount > 0 && (
                    <button
                      type="button"
          ...
          <div ref={profileRef} className="relative ml-1">
            ...
            >
              ...
              <div className="hidden max-w-[130px] text-left lg:block">
                <p className="truncate text-xs font-bold text-slate-700">{user?.full_name || "User"}</p>
              ...
              <ChevronDown
                size={14}
                className={`hidden text-slate-400 transition-transform lg:block ${showProfile ? "rotate-180" : ""}`}
              />
            </button>
                ...
                <div className="mb-1 rounded-xl bg-slate-50 px-3 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 truncate text-[10px] text-slate-400">{user?.email || "Signed in"}</p>
                    </div>
                  </div>
                </div>
                <button
  ...
  return (
    <div
      className={`border-b border-slate-100 px-5 py-4 transition last:border-b-0 hover:bg-slate-50 ${
        !notification.is_read ? "bg-indigo-50/35" : "bg-white"
      }`}
    >
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start gap-2">
            <p className="min-w-0 flex-1 text-xs font-bold leading-5 text-slate-800">{notification.title}</p>
            {!notification.is_read && <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-indigo-600" />}
          </div>
          <p className="mt-1 text-[11px] leading-5 text-slate-500">{notification.message}</p>
          <div className="mt-2.5 flex items-center justify-between gap-3">
            <p className="text-[10px] text-slate-400">{formatNotificationDate(notification.created_at)}</p>
...
function NotificationEmpty() {
  return (
    <div className="px-6 py-10 text-center">
      ...
      <p className="mt-3 text-xs font-bold text-slate-700">No notifications</p>
