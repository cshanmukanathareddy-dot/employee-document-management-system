...
class StorageRequestCreate(BaseModel):
    additional_gb: float = Field(gt=0, le=1024)


def get_storage_usage(user_id: int, db: Session) -> int:
    used = (
...
def get_my_storage(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    allocated = int(current_user.storage_limit_bytes or 0)
    used = get_storage_usage(current_user.id, db)
    ...
    return {
        ...
        "storage_limit_gb": round(allocated / BYTES_PER_GB, 2),
        "storage_used_gb": round(used / BYTES_PER_GB, 4),
        "storage_remaining_gb": round(remaining / BYTES_PER_GB, 4),
        "pending_request": {
            "id": pending.id,
...
def request_more_storage(
    ...
    if current_user.role != "employee":
        raise HTTPException(status_code=403, detail="Only employees can request additional storage.")
    ...
    return {
        "message": "Storage request submitted successfully. An administrator will review it.",
